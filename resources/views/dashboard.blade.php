...
<div class="container-fluid fade-in">
    <div class="card mb-4">
        <div class="card-body" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border-radius: 16px;">
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    ...
                    <p class="mb-0" style="font-size: 1.1rem; opacity: 0.95;">
                        Vai trò: <span style="background: rgba(255,255,255,0.2); padding: 4px 12px; border-radius: 6px; font-weight: 600;">{{ $user->role ?? 'librarian' }}</span>
                    </p>
                </div>
                <a href="{{ route('logout') }}" class="btn btn-danger mt-3 mt-md-0" style="box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('catalog.search') }}" method="GET" class="row g-3">
                <div class="col-md-5">
                    <input type="text" name="query" class="form-control" placeholder="Nhập tiêu đề, tác giả, chủ đề, ISBN..." value="{{ $searchQuery ?? '' }}">
                </div>
                <div class="col-md-4">
                    <select name="search_type" class="form-control">
                        <option value="title" @if(isset($searchType) && $searchType=='title') selected @endif>Theo Tiêu đề</option>
                        <option value="author" @if(isset($searchType) && $searchType=='author') selected @endif>Theo Tên Tác giả</option>
                        <option value="subject" @if(isset($searchType) && $searchType=='subject') selected @endif>Theo Chủ đề</option>
                        <option value="publication_date" @if(isset($searchType) && $searchType=='publication_date') selected @endif>Theo Ngày XB</option>
                    </select>
                </div>
                <div class="col-md-3">
        ...
        <div class="card-body">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-body">
                            <h4 style="color: var(--primary); font-weight: 700; margin-bottom: 8px;">{{ $book->title }}</h4>
    ...
    <div class="row g-4">
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%) !important;">
                    <i class="fas fa-book-open"></i> Cấp phát Sách (Issue Book)
                </div>
                <div class="card-body">
                    <form action="{{ route('librarian.issue') }}" method="POST">
                        @csrf
                        <p class="text-muted small mb-3"><i class="fas fa-info-circle"></i> Issue Book (R7, R8). ID Thành viên/Item</p>
                        <div class="mb-3">
                            <label class="form-label">ID Thành viên</label>
                            <input type="number" name="member_id" class="form-control" placeholder="Nhập ID thành viên" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">ID Book Item</label>
                            <input type="number" name="book_item_id" class="form-control" placeholder="Nhập ID book item" required>
                        </div>
                        <button type="submit" class="btn btn-primary w-100">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('librarian.return') }}" method="POST">
                        @csrf
                        <p class="text-muted small mb-3"><i class="fas fa-info-circle"></i> Return book EXTEND Pay fine (R12)</p>
                        <div class="mb-3">
                            <label class="form-label">ID Lần mượn (Book Lending ID)</label>
                            <input type="number" name="lending_id" class="form-control" placeholder="Nhập ID lần mượn" required>
                        </div>
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header" style="background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%) !important;">
                    <i class="fas fa-plus-circle"></i> Thêm Sách Mới
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.books.create') }}" method="POST">
                        @csrf
                        <p class="text-muted small mb-3"><i class="fas fa-info-circle"></i> Add book INCLUDE Add book item</p>
                        <div class="row g-2">
                            ...
                            <div class="col-md-6 mb-2">
                                <input type="number" name="author_id" class="form-control" placeholder="ID Tác giả" required>
                            </div>
                            <div class="col-md-6 mb-2">
                                <input type="number" name="num_copies" class="form-control" placeholder="Số lượng bản sao" required min="1">
                            </div>
                            <div class="col-md-6 mb-3">
                                <input type="number" name="rack_id" class="form-control" placeholder="ID Kệ sách" required>
                            </div>
                        </div>
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header" style="background: linear-gradient(135deg, #fa709a 0%, #fee140 100%) !important;">
                    <i class="fas fa-user-plus"></i> Đăng ký Thành viên Mới
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.members.register') }}" method="POST">
                        @csrf
                        <p class="text-muted small mb-3"><i class="fas fa-info-circle"></i> Register new account INCLUDE Issue library card</p>
                        <div class="row g-2">
                            <div class="col-md-6 mb-2">
    ...
    <div class="row g-4">
        <div class="col-md-4">
            <div class="card">
                <div class="card-body text-center">
                    ...
                    <p class="text-muted mb-4">Số sách đã mượn: {{ $user->lendings()->whereNull('return_date')->count() }} / 10 (R7)</p>
            ...
            <div class="card">
                <div class="card-body">
                    <h5 style="font-weight: 700; margin-bottom: 16px;">
                        <i class="fas fa-clock"></i> Gia hạn Sách (R11)
                    </h5>
                    <form action="{{ route('member.renew', ['lending' => 1]) }}" method="POST">
                        @csrf
                        <p class="text-muted small mb-3">Gia hạn sách đang mượn.</p>
                        <div class="mb-3">
                            <input type="number" name="lending_id" class="form-control" placeholder="ID Lần mượn (đang mượn)" value="1" required>
                        </div>
                        <button type="submit" class="btn btn-warning w-100 text-white">
                            <i class="fas fa-sync-alt"></i> Gia hạn (Mock ID)
                        </button>
                    </form>
                </div>
            </div>
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-body text-center">
                    <i class="fas fa-history" style="font-size: 3rem; color: #10b981; margin-bottom: 16px;"></i>
